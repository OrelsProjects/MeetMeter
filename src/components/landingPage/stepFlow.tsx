...
const Card = ({ icon, title, body }: CardItem) => (
  <div className="h-72 !w-full md:w-72 card">
...
const StepFlow: React.FC<StepFlowProps> = ({ items }) => {
  return (
    <div className="md:max-w-full flex flex-col md:flex-row md:justify-between items-center gap-4 md:gap-8">
      {items.map((item, index) => (
        <ShowContentContainer
          variant="main"
          key={`walkthrough-card-${index}`}
          className="!w-[360px] flex flex-col md:flex-row gap-4 md:gap-8 justify-start items-center"
        >
          <Card {...item} />
          <FaArrowRightLong
            className={cn(
